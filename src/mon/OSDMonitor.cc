...
bool OSDMonitor::preprocess_command(MonOpRequestRef op) {
    ...
    if (prefix == "osd stat") {
        ...
        const choices_map_t ALL_CHOICES = {
                ...
                {"eio",                           POOL_EIO},
                {"allow_ec_overwrites",           EC_OVERWRITES},
                {"nodelete",                      NODELETE},
...
bool OSDMonitor::prepare_set_flag(MonOpRequestRef op, int flag) {
    ...
    wait_for_finished_proposal(op, new AbstractMonitor::C_Command(mon, op, 0, ss.str(),
...
bool OSDMonitor::prepare_command_impl(MonOpRequestRef op,
    ...
    if (prefix == "osd setcrushmap" ||
        ...
        do {
            ...
            if (!newcrush.check_item_loc(cct, id, loc, (int *) NULL)) {
                ...
                ss << "no need to move item id " << id << " name '" << name << "' to location " << loc
                   << " in crush map";
                err = 0;
            }
        } while (false);
        ...
        wait_for_finished_proposal(op,
                                   new AbstractMonitor::C_Command(mon, op, err, ss.str(),
                                                                  get_last_committed() + 1));
        return true;
        ...
        wait_for_finished_proposal(op, new AbstractMonitor::C_Command(mon, op, err, ss.str(),
                                                                      get_last_committed() + 1));
        return true;
        ...
        if (any) {
            getline(ss, rs);
            wait_for_finished_proposal(op, new AbstractMonitor::C_Command(mon, op, err, rs,
                                                                          get_last_committed() + 1));
            return true;
        }
        ...
        if (pending_inc.old_pools.count(pgid.pool())) {
            ...
            getline(ss, rs);
            wait_for_finished_proposal(op,
                                       new AbstractMonitor::C_Command(mon, op, err, rs, get_last_committed() + 1));
            return true;
        }
        ...
        wait_for_finished_proposal(
                op,
                new AbstractMonitor::C_Command(mon, op, 0, rs, rdata, get_last_committed() + 1));
        return true;
     ...
     */
        ...
        wait_for_finished_proposal(op,
                                   new AbstractMonitor::C_Command(mon, op, 0, rs, rdata,
                                                                  get_last_committed() + 1));
        force_immediate_propose();
        return true;
        ...
        getline(ss, rs);
        wait_for_finished_proposal(op, new AbstractMonitor::C_Command(mon, op, ret, rs,
                                                                      get_last_committed() + 1));
        return true;
        ...
        if (p->read_tier == tierpool_id) {
            ss << "tier pool '" << tierpoolstr << "' is the overlay for '" << poolstr
