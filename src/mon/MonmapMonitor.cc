...
void MonmapMonitor::on_active() {
    if (get_last_committed() >= 1 && !mon.has_ever_joined) {
         ...
         */
        auto t(std::make_shared<MonitorDBStore::Transaction>());
        t->put(AbstractMonitor::MONITOR_NAME, "joined", 1);
        mon.store->apply_transaction(t);
    ...
    if (prefix == "mon add") {
        ...
        if (!mon.osdmon()->is_readable()) {
            dout(10) << __func__
                     ...
                     << dendl;
            mon.osdmon()->wait_for_readable(op, new AbstractMonitor::C_RetryMessage(&mon, op));
            return false;
        }
        ...
        }
        {
            ...
            struct Plugger {
                ...
                Plugger(SMRProtocol &p) : p(p) { p.plug(); }

                ~Plugger() { p.unplug(); }
            } plugger(smr_protocol);
