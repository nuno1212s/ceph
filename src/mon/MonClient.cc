...
MonClient::MonClient(CephContext *cct_, boost::asio::io_context &service, uint32_t connections_to_maintain) :
        Dispatcher(cct_),
        AuthServer(cct_),
        messenger(NULL),
        ...
        connections_to_maintain(connections_to_maintain),
...
int MonClient::get_monmap_and_config() {
    ...
    while (tries-- > 0) {
        ...
        r = authenticate(std::chrono::duration<double>(
...
void MonClient::send_mon_message2(MessageRef m) {
