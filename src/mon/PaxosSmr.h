...
public:
     ...
     */
    bool is_active() const override { return state == STATE_ACTIVE; }
     ...
     */
    bool is_updating() const override { return state == STATE_UPDATING; }
    ...
    void wait_for_readable(MonOpRequestRef op, Context *c, version_t ver) override {
        ceph_assert(!is_readable(ver));
        if (op)
            op->mark_event("paxos:wait_for_readable");
        waiting_for_readable.push_back(c);
    }

    void wait_for_writeable(MonOpRequestRef op, Context *c) override {
